...
func _ready() -> void:
	scythe_mesh.mesh = null  # Hide debug geometry; trail + slash sprites provide visual feedback
	...
	if _model_scene:
		...
		if ResourceLoader.exists(_sprite_path):
			...
			if _pa:
				var _wdata = WeaponDB.get_weapon("scythe")
...
func _spawn_slash_trail(pos: Vector3) -> void:
	WeaponVFX.spawn_slash_trail(self, _slash_tex, pos, 0.03, 1.2, 0.15)
