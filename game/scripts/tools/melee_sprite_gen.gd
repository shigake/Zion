...
func _gen_lance_thrust() -> void:
	...
	var gold = Color(0.85, 0.75, 0.2, 0.8)
	var gold_bright = Color(1.0, 0.9, 0.4, 0.9)

	# Central thrust line (wider)
	_fill(img, 26, 4, 12, 52, silver)
	_fill(img, 28, 6, 8, 48, silver_bright)
	_fill(img, 30, 8, 4, 44, Color(1.0, 1.0, 1.0, 1.0))
	# Side streaks (multiple)
	_fill(img, 22, 14, 2, 38, silver_dark)
	_fill(img, 40, 14, 2, 38, silver_dark)
	_fill(img, 20, 20, 2, 28, Color(0.5, 0.5, 0.6, 0.4))
	_fill(img, 42, 20, 2, 28, Color(0.5, 0.5, 0.6, 0.4))
	# Pointed tip (sharper, larger)
	_fill(img, 29, 2, 6, 2, silver)
	_fill(img, 30, 0, 4, 2, silver_bright)
	_px(img, 31, 0, Color(1.0, 1.0, 1.0, 1.0))
	_px(img, 32, 0, Color(1.0, 1.0, 1.0, 1.0))
	# Arrow tip wings (gold)
	_fill(img, 22, 6, 6, 3, gold)
	_fill(img, 36, 6, 6, 3, gold)
	_fill(img, 24, 5, 4, 1, gold_bright)
	_fill(img, 36, 5, 4, 1, gold_bright)
	# Speed lines (more of them, varied)
	_line(img, 16, 18, 20, 52, Color(0.7, 0.7, 0.8, 0.35))
	_line(img, 48, 18, 44, 52, Color(0.7, 0.7, 0.8, 0.35))
	_line(img, 14, 24, 18, 56, Color(0.6, 0.65, 0.75, 0.25))
	_line(img, 50, 24, 46, 56, Color(0.6, 0.65, 0.75, 0.25))
	_line(img, 12, 30, 16, 58, Color(0.55, 0.6, 0.7, 0.2))
	_line(img, 52, 30, 48, 58, Color(0.55, 0.6, 0.7, 0.2))
	# Impact flash at tip
	_circle(img, 32, 4, 3, Color(1.0, 0.95, 0.8, 0.7))
